...
void MainWindow::connectMapWgtSignal()
{
    ...
    connect(m_pMapWgt, &MapWidget::WPDeleted, this, &MainWindow::on_delWPItem);
    connect(m_pMapWgt, &MapWidget::signalClearWP, this, &MainWindow::on_clearWP);
}
...
//           <<"lng:"<<wpItem->Coord().Lng();
    ...
    connect(wpItem,&WayPointItem::WPValuesChanged,wgt,&WayPointItemWgt::setInfo);
    wgt->setTitle(QString("航点%1").arg(num));
    wgt->setInfo(wpItem);
    m_pWayPointWgt->addWPItem(wgt);
}

void MainWindow::on_delWPItem(int num)
{
    m_pWayPointWgt->delWPItem(num);
}

void MainWindow::on_clearWP()
{
    m_pWayPointWgt->clearWPItem();
}
